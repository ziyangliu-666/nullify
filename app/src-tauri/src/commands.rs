...
pub fn write_settings(
    ...
    let bhop_restore_fps = read_bhop_restore_fps(&cfg_dir);
    let content = render_settings(&settings, &bhop_restore_fps);
...
fn read_bhop_restore_fps(cfg_dir: &str) -> String {
    let path = PathBuf::from(cfg_dir).join("config.cfg");

    fs::read_to_string(path)
        .ok()
        .and_then(|content| parse_fps_max(&content))
        .unwrap_or_else(|| "0".to_string())
}

fn parse_fps_max(content: &str) -> Option<String> {
    let mut fps_max = None;
    ...
    for line in content.lines() {
        ...
        if !lower.starts_with("fps_max") {
            continue;
        }

        let raw_value = trimmed["fps_max".len()..]
            .trim()
            .trim_end_matches(';')
            .trim()
            .trim_matches('"');
        ...
        if raw_value.chars().all(|c| c.is_ascii_digit() || c == '.') {
            fps_max = Some(raw_value.to_string());
        }
    }
...
fn render_settings(s: &Settings, bhop_restore_fps: &str) -> String {
    format!(
        "// ============================================================\n\
         ...
         \n\
         // --- Bhop FPS ---\n\
         alias bhop_restore_fps \"fps_max {bhop_restore_fps}\"\n\
         \n\
         // --- Features ---\n\
         {bhop}\n\
        {jumpbug}\n\
        ...
        bhop_restore_fps = bhop_restore_fps,
